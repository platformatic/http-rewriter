...
mod tests {
    ...
    fn test_fluent_api() {
        ...
        let rewriter = PathRewriter::new("^/api/(.*)", "/v2/$1")
            .unwrap()
            .when(MethodCondition::new(Method::GET).expect("Method::GET is always valid"));
    ...
    fn test_chained_rewriters() {
        ...
        let rewriter = PathRewriter::new("^/old/(.*)", "/new/$1")
            .unwrap()
            .then(MethodRewriter::new(Method::POST).expect("Matching method should be valid"))
            .then(|mut request: Request<()>| {
                request
                    .headers_mut()
                    .insert("X-Rewritten", "true".parse().unwrap());
                Ok(request)
            });
    ...
    fn test_closure_condition_preserves_body() {
        ...
        let is_api_path =
            |request: &Request<()>| -> bool { request.uri().path().starts_with("/api/") };

        // Create a rewriter with the closure condition
        let rewriter = PathRewriter::new("^/api/v1/(.*)$", "/api/v2/$1")
            .unwrap()
            .when(is_api_path);
    ...
    fn test_existence_conditions_with_document_root() {
        ...
        request.set_document_root(temp_dir.as_path().display().to_string().into());
        assert!(exists_cond.matches(&request));
        ...
        let mut request2 = Request::builder()
            ...
            .body(Bytes::new())
            .unwrap();
        request2.set_document_root(temp_dir.as_path().display().to_string().into());
