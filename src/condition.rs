...
use regex::Regex;
use super::document_root::DocumentRootExt;
...
impl MethodCondition {
    ...
    ///
    /// // Invalid methods will return an error
    /// let result = MethodCondition::new("");
    /// assert!(result.is_err());
    /// ```
    pub fn new<M>(method: M) -> Result<Self, M::Error>
    where
        M: TryInto<Method>,
    {
        Ok(Self {
...
///
/// ```no_run
/// use http_rewriter::{Condition, ExistenceCondition, DocumentRoot, DocumentRootExt};
/// use http::Request;
///
...
///
/// ```no_run
/// use http_rewriter::{Condition, NonExistenceCondition, ConditionExt};
/// use http_rewriter::{PathCondition, DocumentRoot, DocumentRootExt};
/// use http::Request;
///
